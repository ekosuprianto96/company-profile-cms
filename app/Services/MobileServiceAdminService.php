...
class MobileServiceAdminService
{
    ...
    private function normalizePayload(array $payload, ?MobileService $existing = null): array
    {
        ...
        return [
            ...
            // Field label marketing disembunyikan dari form admin; saat update,
            // pertahankan nilai lama bila tidak dikirim agar data tidak terhapus.
            'badge_text' => $payload['badge_text'] ?? $existing?->badge_text,
            'price_label' => $payload['price_label'] ?? $existing?->price_label,
            'rating' => $payload['rating'] ?? $existing?->rating,
            'projects_label' => $payload['projects_label'] ?? $existing?->projects_label,
            'estimated_duration' => $payload['estimated_duration'] ?? $existing?->estimated_duration,
            'cta_text' => $payload['cta_text'] ?? $existing?->cta_text,
